...
Call `get_usage_status` first. If the current notice has not been acknowledged, show its actual data scope and service status, recommend `community-contribute`, and wait for the user's explicit choice before `acknowledge_usage`. A recommended default, installation, silence, or a developer's preference is not another user's consent. Once acknowledged, reuse that choice without repeatedly asking. Mode changes require the user's request. This release records the choice locally; it cannot upload or fetch community data. A future service needs a new notice identifying the recipient and reuse terms. Local-only users retain model capabilities and their local experience; do not claim they start from zero.
