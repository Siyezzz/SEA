...
For candidate comparison, read [the algorithm contract](../../docs/algorithms.md) and run `python evolution.py REPORT.json` from the repository root on externally observed paired outcomes. This produces an offline eligibility decision, an observed Pareto frontier, and a suggested experiment operator. It does not execute a candidate or prove the supplied evidence authentic. Preserve the fixed evaluation plan and account for repeated comparisons. Read [related work](../../docs/related-work.md) when choosing a new learning mechanism; treat published algorithms as testable options rather than mandatory additions.
