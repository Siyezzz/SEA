...
SEA's server speaks standard MCP over local STDIO. The Codex plugin bundles a host-specific skill and manifest; `sea_mcp.py` itself has no Codex dependency. A host that can launch a Python subprocess and call MCP tools can connect. The optional community registry is HTTPS, but it is a data service used by the local MCP server rather than a remote MCP endpoint.
...
The host chooses its own model. Several hosts owned by the same user can point to one local database; different owners should use separate databases. SQLite remains private local storage. To enable community exchange, add `--registry-url`, `--client-id`, and `--client-secret-env` to the server arguments and expose that environment variable to the MCP process. The configuration generator supports the same flags and never prints the secret value.
