...
SEA uses an original filled heart-wave mark: a blue-to-cyan heart crossed by a thick ocean curve with a warm reflection on deep ocean blue. It connects learning with care for the community feedback loop. `assets/sea-icon.svg` is the scalable source, `assets/sea-icon.png` is the Codex plugin image, and `assets/sea-social.png` is the repository banner/social-card asset. The same vector geometry is embedded in the Registry page. The mark is embedded in the GitHub README. The social-card file does not itself configure GitHub's repository social-preview setting. `scripts/build_brand.py` regenerates the images using Pillow as a build-only dependency.
